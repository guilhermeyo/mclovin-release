...
La misión es cubrir los tres sistemas (Linux ya está, macOS llega en `0.2`, Windows en `0.3`) y, en el camino, exponer lo que los sistemas operativos y los navegadores llevan demasiado tiempo sin resolver. Haces clic en un link y se abre en el lugar correcto. Sin preguntar qué profile, sin cerrar la ventana incorrecta, sin perder dos minutos por cada link.
...
mclovin es un único binario Rust con tres frontends que comparten el mismo core: CLI (scripts), TUI (`mclovin menu` en la terminal) y GUI (`mclovin settings`). Las reglas se comportan igual en cualquier frontend porque viven en disco en `rules.toml`.
...
[picker]
enabled = true
# command = "tui"   # opcional: abre el picker como ventana TUI (alacritty)
                    # default: picker GUI iced
```
...
| **TUI Menu** | `mclovin menu` en una terminal | Cuando ya estás en terminal y prefieres ratatui |
| **Picker** | Aparece al hacer clic en un link sin regla (Ctrl+Shift+B en Hyprland) | Elección rápida de browser por URL |

### Atajos

Tanto en la GUI como en la TUI:
