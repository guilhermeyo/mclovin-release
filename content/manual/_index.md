...
mclovin is a single Rust binary with two frontends sharing the same core: CLI (scripts) and GUI (`mclovin settings`). Rules behave the same in either frontend because they live on disk in `rules.toml`.
...
[picker]
enabled = true
# command = ""   # default: iced GUI picker; or set to fuzzel/walker/wofi/rofi/bemenu
```
