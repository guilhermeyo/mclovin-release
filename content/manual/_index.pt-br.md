...
O mclovin é um único binário Rust com dois frontends compartilhando o mesmo core: CLI (scripts) e GUI (`mclovin settings`). As regras são as mesmas em qualquer frontend porque vivem em disco no `rules.toml`.
...
[picker]
enabled = true
# command = ""   # padrão: picker GUI iced; ou defina como fuzzel/walker/wofi/rofi/bemenu
```
